...
var Article = React.createClass({
    ...
    onFinishClick: function() {
        ...
        this.props.data.timeOut = '';
        //обнулила счетчики и убрала кнопки
        this.setState({
            clockOutHide: true
        });
        this.setState({
    ...
    render: function() {
        ...
        return ( < div className = "article" >
            < p className = "news_author" > {
                author
            }: < /p> < p className = "news_text" > {
            text
        } < /p>

        { /* для ссылки readmore: не показывай ссылку, если visible === true */ } < a href = "#"
        onClick = {
            this.readmoreClick
        }
        className = {
            "news_readmore" + (visible ? 'none' : '')
        } > Description < /a>

        { /* для большо текста: не показывай текст, если visible === false */ } < p className = {
            "news_bigText" + (visible ? '' : 'none')
        } > {
            bigText
        } < /p> < button className = 'clock'
        onClick = {
            this.onClockClick
        }
        disabled = {
            ...
            Clock In < /button> < button className = 'clock'
        onClick = {
        ...
        disabled = {
            ...
            Clock Out < /button> < button className = 'clock'
        onClick = {
            this.onFinishClick
        }
        disabled = {
...
var Add = React.createClass({
    ...
    render: function() {
        ...
        return ( < form className = 'add cf' >
            ...
            onChange = {
                this.onFieldChange.bind(this, 'authorIsEmpty')
            }
            placeholder = 'Name'
            ref = 'author' / >
            < input type = 'text'
            className = 'add_text'
            onChange = {
                this.onFieldChange.bind(this, 'textIsEmpty')
            }
            placeholder = 'Priority'
            ref = 'text' / >
            < textarea className = 'add_bigText'
            onChange = {
                this.onFieldChange.bind(this, 'bigTextIsEmpty')
            }
            placeholder = 'Task description'
            ref = 'bigText' >
            < /textarea> < label className = 'add_checkrule' > < input type = 'checkbox'
            ref = 'checkrule'
            onChange = {
            ...
            ref = 'add_button'
            disabled = {
                agreeNotChecked || authorIsEmpty || textIsEmpty || bigTextIsEmpty
            } >
...
var App = React.createClass({
    ...
    render: function() {
        ...
        return ( < div className = "app" >
            < h3 > TASKS < /h3> < Add / > { /*Добавила вывод компонента*/ } < News data = {
                this.state.news
            }
            /> < /div >
        );
